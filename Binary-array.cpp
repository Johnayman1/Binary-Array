...
void binary_array::runFromTerminal(){
    ...
    // Input array size
    while (true) {
        cout << "Enter the number of elements in the array between 1 and 99: ";
        cin >> input;
    ...
    // Input size of subarray to flip
    while (true) {
        cout << "Enter the size of subarray to flip (non-negative): ";
        cin >> input;

        if (isPositiveInteger(input)) {
            this->sizeOfSub = stoi(input);
            break;
        }
